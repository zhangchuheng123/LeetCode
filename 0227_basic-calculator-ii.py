...
class Solution:
    ...
    @staticmethod
    def to_postfix(chars):
        expr_stack = []
        op_stack = []
        last_num = False
        for ch in chars:
            if ch in ['+', '-', '*', '/']:
                while len(op_stack) > 0 and priority[op_stack[-1]] >= priority[ch]:
                    expr_stack.append(op_stack.pop())
                op_stack.append(ch)
                last_num = False
            else:
                if last_num:
                    expr_stack[-1] = expr_stack[-1] * 10 + int(ch)
                else:
    ...
    @staticmethod
    def calculate_postfix(expr_stack):
        calc_stack = []
        for expr in expr_stack:
            if type(expr) is str:
                ...
                if expr == '+':
                    ...
                    num3 = num2 // num1
                calc_stack.append(num3)
            else:
                calc_stack.append(expr)
        return calc_stack[-1]

    def calculate(self, s):
        chars = [ch for ch in s if ch != ' ']
        expr_stack = self.to_postfix(chars)
        return self.calculate_postfix(expr_stack)

if __name__ == '__main__':
    print(Solution().calculate(' 3-5+2'))
    print(Solution().calculate(' 3+5 / 2 +2- 1*3+3'))